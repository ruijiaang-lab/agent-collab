...
打开 `http://127.0.0.1:5057/chair`，你会看到一个三栏布局的会议控制台：

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  圆桌会议控制台    [任务] [极简] [高级] [设置]    [自动模式] [生成结论] [刷新] │
├────────────┬────────────────────────────────────┬───────────────────────────┤
│            │                                    │                           │
│  主席席位  │  当前会议                           │  ▼ 主席指令               │
│  ┌──────┐  │  ┌──────────────────────────────┐  │  ┌─────────────────────┐  │
│  │ 主席  │  │  │ 议题：如何优化登录流程        │  │  │ 禁止修改数据库 schema │  │
│  │  你   │  │  │ 阶段：圆桌讨论  轮次：2      │  │  └─────────────────────┘  │
│  └──────┘  │  │ 发言权：Claude Code           │  │                           │
│            │  └──────────────────────────────┘  │  ▼ 提案与裁决             │
│  参会席位  │                                    │  ┌─────────────────────┐  │
│  ┌──────┐  │  ┌──────────────────────────────┐  │  │ 用 JWT 替换 session │  │
│  │ Codex│  │  │ 泳道视图 | 时间线             │  │  │ 提案人: Claude Code │  │
│  │ 就绪 │  │  ├──────┬───────┬───────┬──────┤  │  │ 票型: 2赞成 1反对   │  │
│  └──────┘  │  │ 主席  │ Codex │Claude │Hermes│  │  │ 状态: 待裁决        │  │
│  ┌──────┐  │  │      │       │ Code  │      │  │  └─────────────────────┘  │
│  │Claude│  │  │ ┌──┐ │ ┌──┐  │ ┌──┐  │      │  │                           │
│  │Code  │  │  │ │R1│ │ │R1│  │ │R2│  │ 排队 │  │  ▶ 行动项 (折叠)         │
│  └──────┘  │  │ └──┘ │ └──┘  │ └──┘  │      │  │  ▶ 交接   (折叠)         │
│  ┌──────┐  │  └──────┴───────┴───────┴──────┘  │                           │
│  │Hermes│  │                                    │                           │
│  │ 就绪 │  │  ┌──────────────────────────────┐  │                           │
│  └──────┘  │  │ [主席] [Codex] [Claude] [Hermes]│ │                         │
│            │  │ [观点] [质询] [风险] [执行计划]│  │                           │
│  会议规则  │  │ 输入本轮发言...                │  │                           │
│  ...       │  │                    [提交发言]  │  │                           │
│            │  └──────────────────────────────┘  │                           │
...
在右侧「主席指令」区域，告诉 AI 什么能做、什么不能做：
- 例：「禁止修改数据库 schema」「必须考虑向后兼容」
- 指令优先级最高，所有 Agent 必须遵守

**第 3 步：让 AI 发言**

两种方式：
- **手动**：在底部发言区选 Agent + 立场（观点/质询/风险/执行计划），写内容，点「提交发言」
- **自动**：开启顶栏「自动模式」，设置轮数，floor 切到哪个 Agent 就自动唤醒它

**第 4 步：裁决提案**

当 Agent 提交提案后，右侧「提案与裁决」会显示：
- 查看票型（Agent 可投票：赞成 / 反对 / 弃权）
- 点击提案卡片，选择：**通过** / **否决** / **暂缓** / **重议**
- 否决时填写理由 → 系统自动追加为高优指令并重新唤醒该 Agent

**第 5 步：生成结论**

讨论完毕后，点顶栏「生成结论」：
- 选一个 Agent 来综合（推荐 Hermes）
- 选输出格式：结论纪要 / 行动项清单 / 对外周报
- 20-60 秒后结论出现在会议区域，可一键复制

### 侧栏面板说明

| 面板 | 用途 | 默认状态 |
|---|---|---|
| **主席指令** | 下达约束、禁止事项、验收标准 | 展开 |
| **提案与裁决** | Agent 提案 + 主席裁决 + 投票 | 展开 |
| **行动项** | 创建任务、分配负责人 | 折叠 |
| **交接** | 导出当前状态为 Markdown | 折叠 |

### 泳道视图 vs 时间线

右上角可切换两种发言视图：
- **泳道视图**（默认）：每个 Agent 一条轨道，发言卡片按轮次排列，当前发言权高亮
- **时间线**：所有发言按时间顺序排列，适合回溯完整讨论过程
...
| 修改 AI 配置 | 顶栏「设置」 |
| 开启自动发言 | 顶栏「自动模式」勾选 |
| 唤醒某个 Agent | 泳道视图里点 Agent 卡片上的「唤醒」按钮 |
| 投票 | 提案卡片内的赞成/反对/弃权按钮 |
| 复制结论 | 结论卡片右上角「复制」按钮 |
...
1. **新建任务** — 描述你想让 AI 帮你做的事，选"圆桌讨论"或"拆解执行"
2. **看 AI 干活** — 每个 AI 的产出实时可见，状态自动更新
3. **拿走结论** — 一键复制或下载 Markdown
4. **继续追问** — 在结论基础上继续讨论，或单独问某一个 AI
5. **审核** — 可选开启审核机制，通过 / 打回
...
| 模式 | 流程 | 适用场景 |
|---|---|---|
| **圆桌讨论** | AI 轮流发言 → 自动结论 | 开放性问题、多角度分析 |
| **拆解执行** | Hermes 拆解 → 分发 → 合成 | 复杂工程任务、需要并行执行 |

设置页（`/settings`）可以配置每个 AI 用 CLI 模式（复用本地登录）还是 API 模式（填第三方端点）。

> **v0.6.1**：所有页面顶部统一导航栏（任务 / 极简 / 高级 / 设置），高级模式侧栏面板默认折叠，界面更干练。
...
- [x] **v0.6** 任务面板：圆桌讨论 + 拆解执行双模式、继续追问、单独提问、设置页
- [x] **v0.6.1** 全局导航栏 + 高级模式 UI 精简（侧栏折叠、统一四页导航）
